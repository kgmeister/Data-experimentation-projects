    ...
    for p in iter_llm_providers():
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"[Clarity_v3] ❌ {p['name']} failed: {r.status_code} | {r.text[:400]}")

        except Exception as e:
            print(f"[Clarity_v3] {p['name']} exception: {e}")
            continue

    return "[Error] All providers failed. Check console logs."
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port, debug=True)from __future__ import annotations
import os, json, re
from pathlib import Path
from typing import Dict, List, Optional
